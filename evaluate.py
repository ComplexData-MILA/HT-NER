...
parser = argparse.ArgumentParser(description='Evaluate F1 score for each column in entity and token levels.')
parser.add_argument('--ground_truth', type=str, help='Name and location of ground truth CSV file.')
parser.add_argument('--ground_truth_column', nargs='+', help='Names of columns in ground truth CSV file that contain the entities.')
parser.add_argument('--pred', type=str, help='Location of prediction CSV file.')
parser.add_argument('--prediction_column', nargs='+', help='Names of columns in prediction CSV file that contain the entities.')
...
prediction = pd.read_csv(args.pred)
...
for gt_col, pred_col in zip(args.ground_truth_column,
                                  args.prediction_column):
    col = gt_col + " with " + pred_col
    # Extract entities and predicted entities from the current column
    ground_truth_entities = ground_truth[gt_col].fillna('').apply(lambda x: x.lower()).str.split('|').tolist()
    predicted_entities = prediction[pred_col].fillna('').apply(lambda x: x.lower()).str.split('|').tolist()
    
            
    # Initialize the true positives, false positives, and false negatives to 0
    entity_tp[col] = 0 #entity_tp.get(col, 0)
    entity_fp[col] = 0 #entity_fp.get(col, 0)
    entity_fn[col] = 0 #entity_fn.get(col, 0)
    # Initialize the true positives, false positives, and false negatives to 0
    token_tp[col] = 0 #token_tp.get(col, 0)
    token_fp[col] = 0 #token_fp.get(col, 0)
    token_fn[col] = 0 #token_fn.get(col, 0)
...
for entity in entity_tp.keys():
    entity_precision[entity] = entity_tp[entity] / (entity_tp[entity] + entity_fp.get(entity, 0))
    entity_recall[entity] = entity_tp[entity] / (entity_tp[entity] + entity_fn.get(entity, 0))
    entity_f1[entity] = 2 * entity_precision[entity] * entity_recall[entity] / (entity_precision[entity] + entity_recall[entity])
...
for token in token_tp.keys():
    token_precision[token] = token_tp[token] / (token_tp[token] + token_fp.get(token, 0))
    token_recall[token] = token_tp[token] / (token_tp[token] + token_fn.get(token, 0))
    token_f1[token] = 2 * token_precision[token] * token_recall[token] / (token_precision[token] + token_recall[token])
...
entity_matrix = pd.DataFrame({'Precision': entity_precision, 'Recall': entity_recall, 'F1 Score': entity_f1})
