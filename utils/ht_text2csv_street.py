...
label_df = label_df[['ad_id', 'title', 'description', 'location', \
    'label (PN) different entities sep by |', 'Label for title','Street Level', 'City Level', 'Title Street or Other', 'Street or Other']]
label_df = label_df.rename(columns={'label (PN) different entities sep by |':'label', 'Label for title': 'title label',
                                    'Title Street or Other': 'title street mask', 'Street or Other': 'street mask'})
...
pd.DataFrame({'tokens': tokens, 'tags': tags}).to_csv("./data/ht_tokenized_street.csv", index=False)