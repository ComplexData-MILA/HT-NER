...
def postprocess(df, results):
    ...
    for index, res in enumerate(results):
        ...
        try:
            ...
            name = name.strip().replace('Names: ', '').replace('\n','|').replace(', ','|')
            location = location.strip().replace('Locations: ','').replace('\n','|').replace(', ','|')
            social = social.strip().replace('Social: ','').replace('\n','|').replace(', ','|')
        except:
            # if len(res) <= 5: name = location = social = 'N'
            name,location, social = "","",""
            for line in content.split('\n'):
                if 'Names: ' in line: name += line.replace('Names: ','')
                elif 'Locations: ' in line: location += line.replace('Locations: ','')
                elif 'Social: ' in line: social += line.replace('Social: ','')
                elif not line: pass
                else:
                    print(f"Error {index}")
                    print(repr(content))
