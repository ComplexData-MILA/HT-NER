...
class RuleExtractor(Extractor):
    ...
    def define_patterns(self, pnum):
        ...
        weights = {'pattern0':(2, 0.5), 'pattern1':(2, 0.5),'pattern2':(2, 0.5),
               'pattern3':(2, 0.5),'pattern4':(2, 0.5),'pattern5':(1, 0.5),
               'pattern6':(1, 0.5),'pattern7':(1, 0.5),'pattern8':(1, 0.5),
               'pattern9':(1, 0.5),'pattern10':(1, 0.5),'pattern11':(1, 0.5),
               'pattern12':(1, 0.5),'pattern13':(1, 0.5),'pattern14':(1, 0.5),
                'pattern15':(1, 0.5),'pattern16':(2, 0.5),'pattern17':(2, 0.5),
                'pattern18':(2, 0.5),'pattern19':(2, 0.67),'pattern20':(2, 0.5),
                'pattern21':(1, 0.5),'pattern22':(1, 0.5),'pattern23':(1, 0.5),
                'pattern24':(1, 0.5),'pattern25':(1, 0.5),'pattern26':(1, 0.5),}
        if pnum == -1:
...
class DictionaryExtractor(Extractor):
    ...
    def extract(self, text):
        ...
        for _, start, end in matches:
            ...
            ent = Entity(span.text,span.start, self.type)
            try:
                # address key error. If name not found in weights file, then give 0 score
