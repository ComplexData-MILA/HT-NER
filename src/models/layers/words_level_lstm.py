...
def unsorted_segment_sum(data, segment_ids, num_segments, x=None):
    ...
    segment_ids = torch.repeat_interleave(
        segment_ids.unsqueeze(-1), repeats=data.shape[-1], dim=-1
    )
...
def groupby(logits, word_ids, combiner="sum", max_words=28118):
    ...
    logits = unsorted_segment_reduce(
        logits, word_ids_.long(), max_words + 1, combiner=combiner
    )
    return logits[:, 1:]
